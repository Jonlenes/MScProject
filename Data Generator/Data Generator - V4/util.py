...
def get_valid_indexs(x, y, bound):
    valid_idxs1 = np.logical_and((x >= 0), (x < bound))
    valid_idxs2 = np.logical_and((y >= 0), (y < bound))

    return np.where( np.logical_and(valid_idxs1, valid_idxs2) )[0]
...
def possible_angles(radianos, derivative):
    return [elem for elem in radianos]
